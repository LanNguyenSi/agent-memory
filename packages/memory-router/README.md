...
Because the floor is corpus/provider/model-coupled and the flat legacy default (0.5) left every Ollama path effectively unfiltered — a follow-up eval on the (grown, 295-memory) reference corpus measured **0/4 negative controls at the flat 0.5 default**, i.e. junk prompts were injecting memories — `MEMORY_ROUTER_BLEND_MIN_SEMANTIC`'s un-overridden default is now resolved per provider/model instead of being one flat number:
...
Model names are normalized before the lookup (trimmed, lowercased, `:tag` suffix stripped), so an explicit Ollama tag or quantization variant (`bge-m3:567m`) still matches the `bge-m3` row rather than silently falling through to the generic fallback. An explicit `MEMORY_ROUTER_BLEND_MIN_SEMANTIC` **always wins** over this table, on every provider/model path — set it to override the resolved default for further per-corpus tuning, exactly as before. Re-running the eval below with no override, after the fix, measured **4/4 negative controls** and MRR 0.7104 (matching the calibrated-table row above), confirming the conditional default now applies automatically rather than requiring `MEMORY_ROUTER_BLEND_MIN_SEMANTIC=0.78` to be set by hand.
