...
- `MEMORY_ROUTER_HOOK_EMBED_TIMEOUT_MS`: hook-path-only timeout override, precedence over `MEMORY_ROUTER_EMBED_TIMEOUT_MS` above. The index-rebuild path never reads it. Same validation as `MEMORY_ROUTER_EMBED_TIMEOUT_MS` (an invalid value falls back to the next tier, not to an error); see "Timeout budgets" below for why this exists and the full precedence order.
