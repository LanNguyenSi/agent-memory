# Ways of Working: memory-digest-cli
...
```bash
memory-digest-cli run --output json | jq '.items[]'
```
...
```
error: required argument TARGET was not provided. Run 'memory-digest-cli run --help' to see usage.
