...
**memory-digest-cli** is a CLI tool that a cli tool to generate daily memory digests from markdown files, extracting key insights and creating summaries for ai consciousness continuity.

- **Language:** typescript
- **Framework:** commander
- **Distribution:** binary
- **Test strategy:** unit-tests
- **Config format:** json (at `~/.config/memory-digest-cli/config.json`)
...
4. Add an environment variable override: `MEMORY_DIGEST_CLI_<KEY_NAME>`
